...
def add_triple(graph, subject, predicate, object):
    graph.add((subject, predicate, object))


def build_graph():
    # the merged JSON data from FCG Editor outputs and Framester outputs
    merged_data = load_json_file('data/combined_outputs.json')
    ...
    for entry in merged_data:
        ...
        # creating URIs
        frame_FCG = create_uri(framespace, frame_name)
        tweet_uri = create_uri(framespace, tweet_id)
        tweet_date = create_uri(framespace, date)
        tweet_text = create_uri(twtcontent, tweet_id)
        tweet_subjectivity = create_uri(framespace, subjectivity)
        tweet_polarity = create_uri(framespace, polarity)
        tweet_analysis = create_uri(framespace, analysis)

        # creating triples and add to the graph
        add_triple(g_all_tweets, tweet_uri, RDF.type, URIRef('http://example.com/Tweet'))
        add_triple(g_all_tweets, tweet_uri, RDFS.label, Literal(tweet_id))
        # adding tweet date
        ...
        for result in hasFrameRelation_results:
            ...
            g_all_tweets.add(
                (frame_FCG, URIRef('https://w3id.org/framester/framenet/tbox/hasFrameRelation'), relatedFrame_uri))
        ...
        for result in closeMatch_results:
            ...
            g_all_tweets.add(
                (matchedFrames_uri, RDF.type, URIRef('https://w3id.org/framesterpage/wn/wn30/schema/NounSynset')))
        ...
        for result in hasInchoative_results:
            ...
            g_all_tweets.add(
                (frame_FCG, URIRef('https://w3id.org/framesterpage/framenet/tbox/isInchoativeOf'), inchoative_uri))
        ...
        for result in isCausativeOf_results:
            ...
            g_all_tweets.add(
                (frame_FCG, URIRef('http://www.w3.org/framesterpage/framenet/tbox/isCausativeOf'), causativeOf_uri))
        ...
        for result in isInchoativeOf_results:
            ...
            g_all_tweets.add(
                (frame_FCG, URIRef('http://www.w3.org/framesterpage/framenet/tbox/isInchoativeOf'), isInchoativeOf_uri))
        ...
        for result in isPerspectivizedIn_results:
            ...
            g_all_tweets.add((frame_FCG, URIRef('http://www.w3.org/framesterpage/framenet/tbox/isPerspectivizedIn'),
                              perspectivizedIn_uri))
        ...
        for result in narrowerMatch_results:
            ...
            g_all_tweets.add(
                (narrowerMatch_uri, RDF.type, URIRef('https://w3id.org/framesterpage/wn/wn30/schema/AdjectiveSynset')))
            g_all_tweets.add((narrowerMatch_uri, RDFS.label, Literal(narrowerMatchFrameName)))
            g_all_tweets.add(
                (frame_FCG, URIRef('http://www.w3.org/2004/02/skos/core#narrowerMatch'), narrowerMatch_uri))
        ...
        for result in perspectiveOn_results:
            ...
            g_all_tweets.add(
                (frame_FCG, URIRef('https://w3id.org/framesterpage/framenet/tbox/perspectiveOn'), perspectiveOn_uri))
    ...
    # serializing the RDF graph and save it to a file
    with open("data/twitterKnowledgeGraph.ttl", 'wb') as f:
