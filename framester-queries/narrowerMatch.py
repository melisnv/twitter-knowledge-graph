    ...
    if output.get("frameSet") is not None and output["frameSet"] is not None:
        ...
        for frame in frame_set:
            ...
            sparql_template = r'''
                        ...
                        SELECT DISTINCT ?frame ?narrowerMatch (str(?narrowerMatchLabel) as ?label)
                        WHERE {
                          ?frame a tbox:Frame ;
                                 skos:narrowerMatch ?narrowerMatch ;
                                 skos:closeMatch ?closeFrames .
                          ?narrowerMatch rdfs:label ?narrowerMatchLabel .
...
with open('../data/narrowerMatch.json', 'w') as f:
