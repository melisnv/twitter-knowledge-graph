    ...
    if output.get("frameSet") is not None and output["frameSet"] is not None:
        ...
        for frame in frame_set:
            ...
            sparql_template = r'''
                            ...
                            WHERE
                            {{
                                ?frame pbschema:hasRole ?role.
                                FILTER(?frame = <https://w3id.org/framester/pb/pbdata/{frame_name}>)
                                ?role framester:subsumedUnder ?subsumedUnder.
                            }}
                        '''
...
with open('../data/hasRole.json', 'w') as f:
